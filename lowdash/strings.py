...
RE_LATIN1_PATTERN = RegExp("[\xC0-\xFF]", True)
...
def deburr(item) -> str:
    """
    Deburring strings from latin characters
    ```py
    >>> lowdash.strings.deburr("Pokémon")
    >>> "Pokemon"
    """
    return RE_LATIN1_PATTERN.replace(str(item), lambda m: DEBURRED_LETTERS.get(m.group()))